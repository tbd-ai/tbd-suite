...
if [ "$PROFILE" = "true" ] ; then
  profiling_prefix='nv-nsight-cu-cli --profile-from-start off  --export results/core_utilization%p.profile --metric "regex:.*fu_utilization" --target-processes all '
#  profiling_prefix='nvprof --profile-child-processes --profile-from-start off --export-profile results/compute_utilization%p.nvvp --print-summary'
  profiling_postfix="--profile"
fi
...
CMD=" $profiling_prefix $CMD $profiling_postfix "
...
# no need to run phase 2 if we are profiling
if [ "$PROFILE" = "true" ] ; then
  exit
fi 
