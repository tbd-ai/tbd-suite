...
############## TBD INSTRUMENTATION BEGIN ################
    if self.profile_step is not None and self.profile_step % steps_per_loop != 0:
        raise ValueError("profile_step must be a multiple of steps_per_loop")
############## TBD INSTRUMENTATION END ################
...
############## TBD INSTRUMENTATION BEGIN ################
      ...
      if self.profile_step is not None and current_step == self.profile_step:
          logging.info("Starting profile, note that program will exist after this")
          cuda.profile_start()
...
############## TBD INSTRUMENTATION BEGIN ################
      if profile_started:
          logging.info("Stopping profile and exiting")
          cuda.profile_stop()
          exit()
