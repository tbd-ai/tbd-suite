...
def train(hparams, scope=None, target_session=""):
  ...
  prof_start, prof_end = 100, 200

  while global_step < num_train_steps:
    ### Run a step ###
    start_time = time.time()
    try:
      if global_step == prof_start:
        print("====== PROFILE START: global_step=%d" % global_step)
        cuda.profile_start()

      if global_step == prof_end:
        print("====== PROFILE STOP: global_step=%d" % global_step)
        cuda.profile_stop()
        sys.exit(0)
