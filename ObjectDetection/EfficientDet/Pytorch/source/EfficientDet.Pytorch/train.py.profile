...
def train(train_loader, model, scheduler, optimizer, epoch, args):
    ...
    optimizer.zero_grad()
    print("There are %d total iterations" % len(train_loader))
    for idx, (images, annotations) in enumerate(train_loader):
        if idx == 2:
            print("====== PROFILE START, idx=%d" % idx)
            cuda.profile_start()

        if idx == 3:
            print("====== PROFILE STOP, idx=%d" % idx)
            cuda.profile_stop()
            sys.exit(0)
        ...
        if(iteration % 10 == 0):
            print('{} iteration: training ...'.format(iteration), flush=True)
            ans = {
                'epoch': epoch,
                'iteration': iteration,
