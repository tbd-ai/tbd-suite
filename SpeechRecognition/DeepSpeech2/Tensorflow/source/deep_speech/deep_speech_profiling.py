...
class Profiling_Hook(tf.train.SessionRunHook):
    
  def __init__(self,profiling_start_step=100, profiling_end_step=110):
    self.profiling_start_step=profiling_start_step
    self.profiling_end_step=profiling_end_step
  ...
  def after_run(self,run_context,run_values):
    ...
    if self._step==self.profiling_start_step:
      tf.logging.info('---------CUDAprofilestart---------')
      cuda.profile_start()
    if self._step==self.profiling_end_step:
      tf.logging.info('---------CUDAprofilestop---------')
      cuda.profile_stop()
...
import time
class ThroughputHook(tf.estimator.SessionRunHook):
    def __init__(self):
        self.counter = 0
        self.profile = True
        self.end = 0
    def after_run(self, run_context, run_values):
        print("====== ThroughputHook: self.counter=%d" % self.counter)
        self.counter += 1
        if self.profile:
            if self.counter == 2:
                print("====== PROFILE START: counter=%d" % self.counter)
            ...
            if self.counter == 3:
                print("====== PROFILE STOP: counter=%d" % self.counter)
                measure = time.time()-self.end
                print("time for 100 steps:{}".format(measure))
                sys.exit(0)
...
def ctc_loss(label_length, ctc_input_length, labels, logits):
  ...
  return tf.expand_dims(
      tf.nn.ctc_loss(labels=sparse_labels, inputs=y_pred,
                     sequence_length=ctc_input_length,preprocess_collapse_repeated=True),
      axis=1)
...
def run_deep_speech(_):
  ...
  #cuda_prifiling = TestHook()
  #train_hooks.append(cuda_prifiling)

  #Test_Hook = ThroughputHook()
  Test_Hook = TestHook()
  train_hooks.append(Test_Hook)
  ...
  for cycle_index in range(total_training_cycle):
    ...
    #<EcoSys>                                                                                              
    #print('Dataset size:{}; Batch size:{}'.format(len(train_speech_dataset.entries),flags_obj.batch_size))
    #<EcoSys>   
