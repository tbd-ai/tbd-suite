...
def main():
    ...
    # !!!! CHANGED SO PRETAINED WEIGHTS WORK
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                if step % args.log_freq == 0:
                    print("running throughput average: ", args.train_batch_size * args.max_seq_length* (time.time() - train_start)/global_step)
                    dllogger.log(step=(epoch, global_step,), data={"step_loss": final_loss,
                                                                   "learning_rate": optimizer.param_groups[0]['lr'],
                                                                   "running throughput average" : (time.time() - train_start)/global_step})
