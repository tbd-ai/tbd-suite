...
result_dir="./pretrain_profiled_data_$profiler"
...
do
    if [ $profiler = 'nvprof' ]
    then
        prefix="$profiler --profile-child-processes  --profile-from-start off --export-profile  $result_dir/compute_utilization_batch_${batch_size}_%p.nvvp --print-summary"
    else
    ...
    COMMAND="$prefix bash /workspace/bert/tbd_scripts/run_pretraining.sh $batch_size $start_iteration $end_iteration --profile"
	if [ $profiler = 'ncu' ]
	then
