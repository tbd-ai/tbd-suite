...
def parse_arguments():
    ...
    parser.add_argument('--benchmark_start', default=-1, type=int,
            help='iteration to start recording the benchmark info')
    parser.add_argument('--benchmark_stop', default=-1, type=int, 
            help='iteration to stop recording the benchmark info')
        ...
        while True:
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step == args.benchmark_start:
                        print("Benchmarking for {} iterations".format(args.benchmark_stop - args.benchmark_start))
                        if args.profile:
                            cuda.profile_start()

                    if global_step == args.benchmark_stop:
                        if args.profile:
                            cuda.profile_stop()
